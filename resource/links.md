# Links

## Swan Inference

| | |
|---|---|
| Marketplace | [https://inference.swanchain.io](https://inference.swanchain.io) |
| Playground (no account needed) | [https://inference.swanchain.io/playground](https://inference.swanchain.io/playground) |
| Model catalog and prices | [https://inference.swanchain.io/models](https://inference.swanchain.io/models) |
| Pricing and Token Plan | [https://inference.swanchain.io/pricing](https://inference.swanchain.io/pricing) |
| Network status — providers, models, traffic | [https://inference.swanchain.io/network](https://inference.swanchain.io/network) |
| Become a provider | [https://inference.swanchain.io/provider-signup](https://inference.swanchain.io/provider-signup) |
| Provider software (open source) | [https://github.com/swanchain/computing-provider](https://github.com/swanchain/computing-provider) |
| Governance proposals (SIPs) | [https://github.com/swanchain/governance](https://github.com/swanchain/governance) |

API base URL for OpenAI-compatible clients: `https://inference.swanchain.io/v1`. See the [API reference](../bulders/app-developer/swan-inference-api.md).

## Swan Chain

| | |
|---|---|
| Website | [https://www.swanchain.io](https://www.swanchain.io) |
| GitHub | [https://github.com/swanchain](https://github.com/swanchain) |
| Block explorer (mainnet) | [https://swanscan.io](https://swanscan.io) |
| Network reference | [Network Info](../network-reference/readme/README.md) · [Contract Addresses](../network-reference/contract-addresses.md) |
| Business partnership | business@swanchain.io |

## Community

| | |
|---|---|
| Discord | [https://discord.gg/swanchain](https://discord.gg/swanchain) |
| Telegram | [https://t.me/swan\_chain](https://t.me/swan_chain/) |
| X (Twitter) | [https://x.com/swan\_chain](https://x.com/swan_chain) |
| YouTube | [https://www.youtube.com/@swan\_chain](https://www.youtube.com/@swan_chain) |
| LinkedIn | [https://www.linkedin.com/company/swancloud](https://www.linkedin.com/company/swancloud) |
| Medium | [https://swanchain.medium.com/](https://swanchain.medium.com/) |
| Linktree | [https://linktr.ee/swanchain\_official](https://linktr.ee/swanchain_official) |

## Archived (Swan 1.0)

These belong to the pre-inference network and are kept for reference only. See [Legacy: Swan 1.0](../swan-chain-campaign/README.md).

| | |
|---|---|
| Legacy provider dashboard (ECP/FCP) | [https://provider.swanchain.io](https://provider.swanchain.io/overview) |
| Legacy orchestrator | [https://orchestrator.swanchain.io](https://orchestrator.swanchain.io/) |
| Lagrange docs | [https://docs.lagrangedao.org/](https://docs.lagrangedao.org/) |
| Galxe campaign | [https://app.galxe.com/quest/swanchain](https://app.galxe.com/quest/swanchain) |
