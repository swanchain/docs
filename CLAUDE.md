...
- **Swan 2.0 / Inference Cloud** documentation lives in `core-concepts/swan-2.0-inference-cloud/` (a directory: `README.md`, `how-to-use.md`, `become-a-provider.md`, `provider-context-window-faq.md`) and `core-concepts/market-provider/inference-marketplace.md`. Swan 2.0 introduces a dual-token payment model (stablecoins + SWAN tokens), contribution-based rewards replacing UBI, and a unified Computing Provider (CP) role merging ECP/FCP. The existing UBI docs are kept intact and framed as the bootstrap phase; Swan 2.0 sections are additive, not replacements.
